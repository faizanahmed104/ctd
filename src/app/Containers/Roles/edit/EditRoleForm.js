...
import { updateRole, getRole } from "../../../../store/slices/auth";

const EditRoleForm = ({ roleId }) => {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { allRole } = useAppSelector((state) => state.auth);
  ...
  const submitButton = async () => {
    const result = await dispatch(
      updateRole({ roleId, updatedData: RoleData })
    );
  ...
  useEffect(() => {
    const role = allRole.find((role) => role._id === roleId);
    if (role) {
      setRoleData(role);
    } else {
      dispatch(getRole({ page: 1, limit: 10, sortOrder: "desc" }));
    }
  }, [roleId, allRole, dispatch]);

  return (
    <div className="p-6">
      {/* Heading */}
      <div className="mt-4">
        <span className="font-semibold text-2xl text-[#1D1D1D]">Edit Role</span>
