...
} from "../../../../store/slices/clientSubmission";
import {
  HEADERS,
  tableHeaderStyles,
  tableDataStyles,
...
function GoodsAndServices({ category }) {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { submissions, mobileSubmissions, fugitiveSubmissions } =
    useAppSelector(selectClientSubmissionState);
  ...
  const currentSubmissions = categoryData.slice(
  ...
  useEffect(() => {
    setCurrentPage(1);
    dispatch(fetchSubmissions(category));
  }, [dispatch, category]);
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          onClick={() => router.push(`/data/update-data?category=${category}`)}
        >
          Upload Data
        </button>
      </div>
          ...
          <tbody>
            {currentSubmissions && currentSubmissions.length > 0 ? (
              currentSubmissions.map((row, index) => (
                ...
                >
                  {HEADERS.map((header) => (
                    <td key={header.key} className={tdStyles}>
                      {header.key === "actions" ? (
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        {submissions && submissions.length > itemsPerPage && (
          <div className="flex items-center">
            <button
