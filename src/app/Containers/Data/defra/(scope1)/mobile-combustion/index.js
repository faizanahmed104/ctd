...
function MobileCombustion({ category }) {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { mobileSubmissions } = useAppSelector(selectMobileSubmissionState);
  console.log("mobileSubmissions", mobileSubmissions);
  ...
  const totalPages = Math.ceil(mobileSubmissions.length / itemsPerPage) || 1;

  const currentSubmissions = mobileSubmissions.slice(
  ...
  useEffect(() => {
    setCurrentPage(1);
    dispatch(fetchMobileSubmissions(category));
  }, [dispatch, category]);
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          onClick={() => router.push(`/data/defra/mobile-combustion/add`)}
        >
          Upload Data
        </button>
      </div>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        {mobileSubmissions && mobileSubmissions.length > itemsPerPage && (
          <div className="flex items-center">
            <button
