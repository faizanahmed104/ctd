...
import {
  fetchDefraPurchasedElectricitySubmissions,
  selectPurchasedElectricitySubmissionState,
} from "../../../../../../store/slices/defra/purchasedElectricitySubmissions";
import {
  HEADERS,
  tableHeaderStyles,
  tableDataStyles,
...
const PurchasedElectricity = ({ category }) => {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { purchasedElectricitySubmissions } = useAppSelector(
    selectPurchasedElectricitySubmissionState
  );
  ...
  const totalPages =
    Math.ceil(purchasedElectricitySubmissions.length / itemsPerPage) || 1;

  const currentSubmissions = purchasedElectricitySubmissions.slice(
  ...
  useEffect(() => {
    setCurrentPage(1);
    dispatch(fetchDefraPurchasedElectricitySubmissions());
  }, [dispatch]);

  console.log("dd", purchasedElectricitySubmissions);
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          onClick={() => router.push(`/data/defra/${category}/add`)}
        >
          Upload Data
        </button>
      </div>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        {purchasedElectricitySubmissions &&
          purchasedElectricitySubmissions.length > itemsPerPage && (
            <div className="flex items-center">
              <button
