...
} from "../../../../../../store/slices/defra/purchasedHeatSubmissions";
import {
  HEADERS,
  tableHeaderStyles,
  tableDataStyles,
...
const PurchasedHeat = ({ category }) => {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { purchasedHeatSubmissions } = useAppSelector(
    selectPurchasedHeatSubmissionState
  );
  ...
  const totalPages =
    Math.ceil(purchasedHeatSubmissions.length / itemsPerPage) || 1;

  const currentSubmissions = purchasedHeatSubmissions.slice(
  ...
  useEffect(() => {
    setCurrentPage(1);
    dispatch(fetchPurchasedHeatSubmissions());
  }, [dispatch]);
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        {purchasedHeatSubmissions &&
          purchasedHeatSubmissions.length > itemsPerPage && (
            <div className="flex items-center">
              <button
