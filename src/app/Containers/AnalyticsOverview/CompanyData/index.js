...
function CompanyData() {
    const [categories, setCategories] = useState(['2016', '2017', '2018']);
    ...
    const getEntities = async () => {
        try {
            const resp = await fetch(apiUrl + endpoints.analyticsGetEntities);
            const jsonResp = await resp.json();
            console.log('entities received\n', jsonResp);
            setEntities(jsonResp);
        } catch (ex) {
        ...
        <>
            <div className='flex w-full flex-row items-center justify-between bg-[#eaf6f3] py-2 pl-2 pr-[30px] rounded-[5px] mb-[18px]'>
                ...
                <div className='flex flex-row items-center justify-between'>
                    ...
                    />
                    <CustomDropDown options={entities} placeholder='Entity' />
                    {/* <CustomDropDown options={['']} placeholder='Interval' /> */}
                    <CustomDropDown
                        options={['Pie', 'Line', 'Bar']}
                        placeholder='Chart'
                    />
                    <div className='flex items-center gap-2 container-2xl:gap-4 ml-10'>
                        <input
                            type='month'
                            className='block w-full max-w-40 px-4 py-2 text-[#000000] bg-[#BAE2D7] border rounded-[10px] shadow-sm focus:outline-none focus:ring-2 focus:ring-[#86ccb8] text-sm font-normal'
                            placeholder='MM/YYYY'
                            value={startDate}
            ...
            <div className='flex w-full flex-row items-center justify-between bg-[#eaf6f3] py-2 pl-2 pr-[30px] rounded-[5px] mb-[18px]'>
                ...
                <div className='flex flex-row items-center justify-between'>
                    ...
                    <CustomDropDown options={entities} placeholder='Entity' />
                    {/* <CustomDropDown options={['']} placeholder='Interval' /> */}
                    {/* <CustomDropDown options={['Pie','Line','Bar']} placeholder='Chart' /> */}
                </div>
            </div>
            <EmissionByDeft />
        </>
